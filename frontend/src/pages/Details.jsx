      ...
      <section>
        <div className="mb-8 flex flex-col items-start">
          ...
          <Link to="/boats" className="inline">
            <TiDeleteOutline onClick={deleteBoat} className="cursor-pointer text-red-500" size={30} />
          </Link>
        </div>
          ...
          boatsData.reservations.map((item) => {
            return (
              <div className="mb-8" key={item._id}>
                ...
                <p>{item.startDate}</p>
                <p>{item.endDate}</p>
                <TiDeleteOutline onClick={deleteReservation} className="cursor-pointer text-red-500" size={30} />
              </div>
            );
          })}
      </section>
